...
def draw_model(mf_model, xmin, xmax, labels, n_fidelities=3, version='v1', x_fixed=[160,5,40,45,50]):
    SPLIT = 100
    ncol=3
    ...
    colors_fill=['darkturquoise','coral','cadetblue']
    colors_line=['lightseagreen','orangered',"teal",'orangered']
    alphas = [0.1,0.3,0.4,0.2]
    for i in range(0,len(labels)):   
        
        ## Compute mean and variance predictions
        x_plot=[x_fixed[:] for l in range(0,SPLIT)]
        xtmp = np.linspace(xmin[i], xmax[i], SPLIT)
        ...
        for f in range(int(n_fidelities)):
            lf_mean_mf_model, lf_var_mf_model = mf_model.predict(X_plot[f*SPLIT:(f+1)*SPLIT])
            lf_std_mf_model = np.sqrt(lf_var_mf_model)
            ax[indices[i]].fill_between(xtmp.flatten(), (lf_mean_mf_model - lf_std_mf_model).flatten(), 
                        (lf_mean_mf_model + lf_std_mf_model).flatten(), color=colors_fill[f], alpha=0.4)
            ax[indices[i]].plot(xtmp, lf_mean_mf_model, '--', color=colors_line[f])

        ax[indices[i]].set_xlim(xmin[i], xmax[i])
        ax[indices[i]].set_ylabel(r'$y_{CNP}$', fontsize=10)
        ax[indices[i]].tick_params(axis='y')
